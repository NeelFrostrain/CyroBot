...
function execCommand(command) {
    return new Promise((resolve, reject) => {
        exec(command, (error, stdout, stderr) => {
            ...
            if (stderr)
                console.warn(`yt-dlp warning: ${stderr}`); // Just log, don't reject
...
async function downloadYouTubeAudio(videoUrl) {
    try {
        ...
        const { stdout: titleStdout, stderr: titleStderr } = await execCommand(getTitleCommand);
        if (titleStderr)
            console.warn(`yt-dlp warning: ${titleStderr}`);
        const videoTitle = titleStdout.trim().replace(/[^a-zA-Z0-9]/g, "_");
        ...
        if (downloadStderr)
            console.warn(`yt-dlp warning: ${downloadStderr}`);
